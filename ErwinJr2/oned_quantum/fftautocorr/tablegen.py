...
def tablegen(intmax, ps=(2, 3, 5)):
    ...
    """
    log_length = np.log2(intmax)
    allp = 1
    for p in ps:
        compp = p ** (np.arange(log_length / np.log2(p), dtype=np.int32))
        allp = np.outer(allp, compp).reshape(-1)
        allp = allp[allp < intmax]
    return np.sort(allp)


def print_as_c_header(fname, array):
    contents = """/*
...
#endif
""".format(
        filename=os.path.basename(fname).replace(".", "_").upper(),

...
class SchrodingerLayer(object):
    ...
    def _shift_psi(self, psi, n) -> np.ndarray:
        if n == 0:
            ...
            return np.interp(
                self.xPoints - self.periodL * n, self.xPoints, psi, left=0, right=0
            )
    ...
    def _basis_shifter(
        ...
        for n in sorted(ns, reverse=True):
            psisn = np.array(
                [
                    np.interp(self.xPoints - period * n, xPoints, psi, left=0, right=0)
                    for psi in psis0
                ]
            )
    ...
    def _reset_cache(self):
        ...
        self._cacheDipole = [
            [None] * len(self.eigenEs) for _ in range(len(self.eigenEs))
        ]
        self._cacheLO = [[None] * len(self.eigenEs) for _ in range(len(self.eigenEs))]
    ...
    def reset_IFR_cache(self):
        ...
        self._cacheIFR = [[None] * len(self.eigenEs) for _ in range(len(self.eigenEs))]
        self._cacheGamma = [
            [None] * len(self.eigenEs) for _ in range(len(self.eigenEs))
        ]
    ...
    def _solve_whole_ode(self) -> np.ndarray:
        if self.crystalType == "simple":
            self.eigenEs = onedq.cSimpleSolve1D(self.xres, self.Es, self.xVc, self.xMc)
            self.psis = onedq.cSimpleFillPsi(
                self.xres, self.eigenEs, self.xVc, self.xMc
            )
            return self.eigenEs
        xBand = onedq.PyBand(self.crystalType, *self.bandParams)
        self.eigenEs = onedq.cBandSolve1D(self.xres, self.Es, self.xVc, xBand)
        self.psis = onedq.cBandFillPsi(self.xres, self.eigenEs, self.xVc, xBand)
    ...
    def _solve_whole_matrix(self) -> np.ndarray:
        ...
        if self.crystalType == "simple":
            ...
            for n in range(len(self.layerWidths)):
                Indices = np.logical_or.reduce(
                    [
                        ...
                        & (
                            self.xPoints
                            < layerCumSum[n + 1] + k * periodL - self.xres / 2
                        )
                        for k in range(self.repeats)
                    ]
                )
            ...
            self.Hsparse = sparse.diags(
                [self.Hsubd, self.Hdiag, self.Hsubd], [-1, 0, 1], shape=(N, N)
            )
            self.eigenEs, self.psis = splg.eigsh(
                self.Hsparse, self.matrixEigenCount, sigma=self.matrixSigma, tol=1e-8
            )
        ...
        if self.crystalType == "ZincBlende":
            ...
            return self.eigenEs
        raise NotImplementedError(
            "Matrix solver is not implemented " "for {}".format(self.crystalType)
        )
    ...
    def _dipole(self, upper: int, lower: int, shift: int = 0) -> float:
        ...
        if self.solver == "ODE":
            ...
            z = np.trapezoid(
                psi_u * xInvMc_l * np.gradient(psi_l)
    ...
    def _lo_transition(self, upper: int, lower: int, shift: int = 0) -> float:
        ...
        if onedq is None:
            convpsi = fft.irfft(
                np.abs(fft.rfft(self.psi_overlap(upper, lower, shift), 2 * N)) ** 2
            )[:N]
            Iij = (
                2
                * self.xres**2
                * np.trapezoid(exp(-kl * self.xPoints * 1e-10) * convpsi)
            )
        # C implementation
        else:
    ...
    def lo_transition(self, upper: int, lower: int) -> float:
        ...
        if self._cacheLO[upper][lower] is None:
            self._cacheLO[upper][lower] = self._lo_transition(upper, lower)
    ...
    def lo_lifetime(self, state: int) -> float:
        ...
        if self.status == "solved-full":
            try:
                return 1 / sum(
                    np.sum(self._pLO[n][:, self.periodMap[state][0]]) for n in range(3)
                )
            except (TypeError, AttributeError):
                # TypeError is when self.periodMap return None
                # AttributeError is when self.periodMap does not exist
    ...
    def _ifr_transition(
        ...
        for _ in range(self.repeats):
            for n in range(layerN):
                ...
                def interpZ(psi):
                    return (psi[zIdx - 1] * (zn - z2) - psi[zIdx] * (zn - z1)) / (
                        z2 - z1
                    )
    ...
    def ifr_lifetime(self, state: int) -> float:
        ...
        if self.status == "solved-full":
            try:
                return 1 / sum(
                    np.sum(self._pIFR[n][:, self.periodMap[state][0]]) for n in range(3)
                )
            except (TypeError, AttributeError):
                # TypeError is when self.periodMap return None
                # AttributeError is when self.periodMap does not exist
        ...
        for n in self.periodIdx:
            if self.ends[n] < periodIdx / 3:
                ...
                if self.eigenEs[n] < barrierBound[-self.ends[n]]:
                    ...
                    print(
                        "State No.{} is close to the right boundary. "
    ...
    def full_population(self) -> np.ndarray:
        ...
        for i, lower in enumerate(self.periodIdx):
            for j, upper in enumerate(self.periodIdx):
                ...
                for s in (1, -1) if lower == upper else (1, 0, -1):
                    ...
                    self._pIFR[s][j][i], gamma = self._ifr_transition(lower, upper, s)
                    if gamma > 0:
        ...
        self.population = null_space(np.diag(-self.decayRates) + self.transitions)
...
class QCLayers(SchrodingerLayer):
    ...
    def __copy__(self):
        return QCLayers(
            ...
            moleFracs=copy.copy(self.moleFracs),
            xres=self.xres,
            Eres=self.Eres,
            statePerRepeat=self.statePerRepeat,
    ...
    def update_mtrls(self):
        ...
        self.mtrlAlloys = [
            material.Alloy(self.materials[idx], self.moleFracs[idx], self.temperature)
    ...
    def add_mtrl(
        ...
        mtrl: str = None,
        moleFrac: float = None,
        IFRLambda: float = None,
        IFRDelta: float = None,
    ):
        """Add a new material possibility"""
        self.materials.append(mtrl if mtrl else QCMaterial[self.substrate][0])
        self.moleFracs.append(moleFrac if moleFrac else 0.0)
        self.mtrlIFRLambda.append(IFRLambda if IFRLambda else self.mtrlIFRLambda[-1])
        self.mtrlIFRDelta.append(IFRDelta if IFRDelta else self.mtrlIFRDelta[-1])
        self.mtrlAlloys.append(
            material.Alloy(self.materials[-1], self.moleFracs[-1], self.temperature)
        )
    ...
    def del_mtrl(self, n: int):
        ...
        for i in range(len(self.layerMtrls)):
            if self.layerMtrls[i] >= n:
                self.layerMtrls[i] = (
                    self.layerMtrls[i] - 1 if self.layerMtrls[i] > 0 else 0
                )
    ...
    def netStrain(self) -> float:
        ...
        if sum(self.layerWidths) <= 1e-5:
            return -1
        totalStrain = sum(
            self.mtrlAlloys[self.layerMtrls[n]].eps_perp * self.layerWidths[n]
            for n in range(len(self.layerWidths))
        )
    ...
    def populate_material(self):
        ...
        if sum(self.layerWidths) <= 1e-5:
            ...
            sumhwlo = sum(
                self.mtrlAlloys[self.layerMtrls[n]].param["hwLO"] * self.layerWidths[n]
                for n in range(len(self.layerWidths))
            )
            ...
            self.epsrho = np.sum(epsrho[self.layerMtrls] * self.layerWidths) / sum(
                self.layerWidths
            )
        # IFR
        if not self.customIFR:
    ...
    def effective_ridx(self, wl: ScalerOrArray) -> ScalerOrArray:
        ...
        neff = np.average(1 / self.layerRIdx**2, axis=0, weights=self.layerWidths) ** (
            -1 / 2
        )
        return neff
    ...
    def sheet_density(self) -> float:
        ...
        return (
            sum(
                self.layerDopings[n] * self.layerWidths[n]
                for n in range(0, len(self.layerWidths))
            )
            * 1e9
        )
        ...
        for i in range(len(self.periodIdx)):
            ...
            for j in range(i + 1, len(self.periodIdx)):
                ...
                for shift in (-1, 0, 1):
                    ...
                    gamma = (
                        (gamma_para + (self.decayRates[i] + self.decayRates[j]) / 2)
                        * 1e12
                        * hbar
                        / e0
                    )
...
def optimize_layer(qcl: QCLayers, n: int, upper: int, lower: int, iter: int = 50):
...
def optimize_global(qcl: QCLayers, iter: int = 50):
    ...
    for n in range(iter):
        ...
        for n in range(layerNum):
            ...
            print(
                ("   layer No.{}.. Width w={}, " "gain (0:{}, +:{}, -:{})").format(

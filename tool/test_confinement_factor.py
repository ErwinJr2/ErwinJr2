...
def firstGaAs():
    ...
    Gamma = (
        beta
        * np.trapezoid(nn[ac] * (Ey[ac] ** 2 - Ez[ac] ** 2), xs[ac])
        / (np.trapezoid((nn * Ey) ** 2, xs))
    )
    Gamma = Gamma.real
    Gamma0 = (
        beta
        * np.trapezoid(nn[ac] * Ey[ac] ** 2, xs[ac])
        / (np.trapezoid((nn * Ey) ** 2, xs))
    )
    Gamma0 = Gamma0.real
    Gamma1 = (
        beta.real
        * np.trapezoid(nn[ac].real * abs(Ey[ac]) ** 2, xs[ac])
...
def findOutOfPhase():
    ...
    ns = np.array([1.0, 3.28, 2.87, 3.28] + [3.21, 3.28 + 1j] * N + [3.28, 2.87, 3.28])

    # plasmon loss
    plasmonUnit = 8.9698e-5 * wl**2
    nue = 1.0
    gammaUnit = 5.3088e-3
    me = np.array([1, 0.067, 0.15, 0.067] + [0.67, 0.67] * N + [0.067, 0.15, 0.067])
    ...
    Gamma0 = (
        beta
        * sum(np.trapezoid(nn[ac] * Ey[ac] ** 2, xs[ac]) for ac in acs)
        / (np.trapezoid((nn * Ey) ** 2, xs))
    )
    Gamma1 = (
        beta.real
        * sum(np.trapezoid(nn[ac].real * abs(Ey[ac]) ** 2, xs[ac]) for ac in acs)
        / (np.trapezoid(abs(nn.real * Ey) ** 2, xs))
    )
    print(Gamma0, Gamma1)
